...
double calcContactStress(const SpurGear* pinion, const SpurGear* gear, double Wt, double V, double Ko, int Qv, int gearQuality, double Ks, double ZR) {
     ...
     */
     ...
     // Validate Inputs
    if (!pinion || !gear || Wt < 0 || V < 0 || Ko < 1.0 || Ks <= 0 || ZR <= 0 || pinion->pitchDiameter <= 0 || pinion->faceWidth <= 0 || gear->faceWidth <= 0 || pinion->youngsModulus <= 0 || gear->youngsModulus <= 0 || pinion->gearRatio <= 0) {
        return -1.0;
    }
